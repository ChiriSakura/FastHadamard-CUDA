...
static bool parse_args(int argc, char** argv, Config& cfg) {
  for (int i = 1; i < argc; ++i) {
    ...
    else if (key == "--profile") cfg.profile = parse_bool(next());
    else if (key == "--help" || key == "-h") {
      std::printf(
          "Usage: %s [--batch N --seq N --heads N --head_dim N] "
          "[--dtype fp16|bf16 --normalize true|false] "
          "[--warmup N --iters N --seed N --csv PATH --profile true|false]\n",
          argv[0]);
      std::exit(0);
    } else {
...
static bool compare_quant_results(const std::vector<unsigned char>& expected_packed,
                                  const std::vector<float>& expected_scales,
                                  const std::vector<unsigned char>& actual_packed,
                                  const std::vector<float>& actual_scales,
                                  size_t& packed_mismatches,
                                  float& max_scale_abs_error,
                                  float& max_dequant_abs_error) {
  packed_mismatches = 0;
  for (size_t i = 0; i < expected_packed.size(); ++i) {
    if (expected_packed[i] != actual_packed[i]) ++packed_mismatches;
  }
  max_scale_abs_error = 0.0f;
  for (size_t i = 0; i < expected_scales.size(); ++i) {
    max_scale_abs_error = std::max(
        max_scale_abs_error,
        std::fabs(expected_scales[i] - actual_scales[i]));
  }
  max_dequant_abs_error = 0.0f;
  for (size_t i = 0; i < expected_packed.size(); ++i) {
    const int expected_low = (expected_packed[i] & 0x0f) >= 8
                                 ? (expected_packed[i] & 0x0f) - 16
                                 : (expected_packed[i] & 0x0f);
    const int expected_high = (expected_packed[i] >> 4) >= 8
                                  ? (expected_packed[i] >> 4) - 16
                                  : (expected_packed[i] >> 4);
    const int actual_low = (actual_packed[i] & 0x0f) >= 8
                               ? (actual_packed[i] & 0x0f) - 16
                               : (actual_packed[i] & 0x0f);
    const int actual_high = (actual_packed[i] >> 4) >= 8
                                ? (actual_packed[i] >> 4) - 16
                                : (actual_packed[i] >> 4);
    const size_t token = (i * 2) / (expected_packed.size() / expected_scales.size() * 2);
    max_dequant_abs_error = std::max(
        max_dequant_abs_error,
        std::fabs(expected_low * expected_scales[token] -
                  actual_low * actual_scales[token]));
    max_dequant_abs_error = std::max(
        max_dequant_abs_error,
        std::fabs(expected_high * expected_scales[token] -
                  actual_high * actual_scales[token]));
  }
  const float max_scale = *std::max_element(expected_scales.begin(),
                                             expected_scales.end());
  return max_scale_abs_error <= 1.0e-6f &&
         max_dequant_abs_error <= max_scale + 1.0e-6f;
}
...
static void append_csv(const Config& cfg, int total_tokens,
  ...
  if (header) {
    file << "dtype,batch_size,seq_len,num_heads,head_dim,total_tokens,implementation,"
            ...
            "gpu_vs_cpu_quant_within_tolerance,quant_max_abs_error,quant_mae,"
            "quant_rmse\n";
  }
...
int main(int argc, char** argv) {
  ...
  const QuantReference reference = quantize_reference(
    ...
    size_t cpu_packed_mismatches = 0;
    float cpu_max_scale_abs_error = 0.0f;
      float cpu_max_dequant_abs_error = 0.0f;
    const bool cpu_quant_exact = compare_quant_results(
      reference.packed, reference.scales, host_unfused, host_unfused_scales,
        cpu_packed_mismatches, cpu_max_scale_abs_error,
        cpu_max_dequant_abs_error);
  std::printf("[advanced] checks: optimized_vs_baseline=%s "
              ...
              cpu_quant_exact ? "MATCH_WITHIN_TOLERANCE" : "FAIL");
    std::printf("[advanced] gpu_vs_cpu_quant_detail: packed_mismatches=%zu "
                "max_scale_abs_error=%.9e max_dequant_abs_error=%.9e\n",
                cpu_packed_mismatches, cpu_max_scale_abs_error,
                cpu_max_dequant_abs_error);

  if (cfg.profile) CUDA_CHECK(musaProfilerStart());
